...
transmiision_tab_layout = [
    [sg.Text("Select an Image: "), sg.Input(key='--orig-img-file--', size=(40, 1)), sg.FilesBrowse(), sg.Button("Load", key="--orig-img-load--")],
    [sg.Frame(title="Original Image", layout=[[sg.Canvas(key='--orig-img-canvas--')], [sg.Text("Size (bytes): "), sg.Text("0", size=(20, 1), key="--orig-img-size--")]]),
      sg.Frame(title="Recv Image", layout=[[sg.Canvas(key='--recv-img-canvas--')], [sg.Text("Recv. Size (bytes): "), sg.Text("0", size=(20, 1), key="--recv-img-size--")]]) ]
]
...
layout = [
    ...
    [sg.TabGroup(
        [
            ...
            [sg.Tab('Image Transmission', transmiision_tab_layout)]
        ])
    ],
...
window = sg.Window("WiPHY Image Streamer", layout, finalize=True)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if event == '--radio-config-load--':
            streamer = imageStreamer(radio_config=values['--radio-config-file--'])
        ...
        if event == '--orig-img-load--':
            if streamer is not None:
                stat = streamer.load_image(values['--orig-img-file--'])
                if stat:
                    img_stat, img_data = streamer.get_orig_PIL_img()
                    if img_stat:
                        
                        # Plot orig image
                        orig_ax.imshow(img_data['arr'])
                        window['--orig-img-size--'].update(img_data['size'])
                        orig_ax.get_xaxis().set_visible(False)
                        orig_ax.get_yaxis().set_visible(False)
                        draw_figure(window['--orig-img-canvas--'].TKCanvas, orig_img_fig)

                        # Plot recv image
                        recv_img_stat, recv_img  = streamer.get_recv_img()
                        if recv_img_stat:
                            recv_ax.imshow(recv_img)
                            recv_ax.get_xaxis().set_visible(False)
                            recv_ax.get_yaxis().set_visible(False)
                            draw_figure(window['--recv-img-canvas--'].TKCanvas, recv_img_fig)
